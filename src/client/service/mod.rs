...
use crate::client::service::request::{RequestToSv2ClientService, RequestToSv2ClientServiceError};
    ...
    T: Sv2TemplateDistributionClientHandler + Clone + Send + Sync + 'static,
{
    ...
    async fn initiate_connection(
            ...
            Protocol::MiningProtocol => {
                if self.mining_tcp_client.lock().await.is_none() {
                    let config = self.config.mining_config.as_ref().ok_or_else(|| {
                        RequestToSv2ClientServiceError::UnsupportedProtocol {
                    ...
                    let tcp_client = Sv2EncryptedTcpClient::new(config.server_addr, config.auth_pk)
                        .await
                        .ok_or_else(|| {
                            RequestToSv2ClientServiceError::ConnectionError(
            ...
            Protocol::TemplateDistributionProtocol => {
                if self.template_distribution_tcp_client.lock().await.is_none() {
                    let config = self
                        ...
                        .as_ref()
                        .ok_or_else(|| RequestToSv2ClientServiceError::UnsupportedProtocol {
                            protocol: Protocol::TemplateDistributionProtocol,
                        })?;
                    let tcp_client = Sv2EncryptedTcpClient::new(config.server_addr, config.auth_pk)
                        .await
        ...
        let to_str0_255 = |s: String, field: &str| {
            s.clone().into_bytes().try_into().map_err(|_| {
                RequestToSv2ClientServiceError::StringConversionError(format!(
        ...
        match message {
            AnyMessage::Common(CommonMessages::SetupConnectionSuccess(
                ...
                Ok(ResponseFromSv2ClientService::ConnectionEstablished)
            }
            AnyMessage::Common(CommonMessages::SetupConnectionError(setup_connection_error)) => {
                let error_code =
                    String::from_utf8_lossy(setup_connection_error.error_code.inner_as_ref())
                        .to_string();
                Err(RequestToSv2ClientServiceError::SetupConnectionError(
                    error_code,
                ))
            }
            _ => Err(RequestToSv2ClientServiceError::ConnectionError(
    ...
    pub async fn listen_for_messages(
        &mut self,
        protocol: Protocol,
    ) -> Result<(), RequestToSv2ClientServiceError> {
        if !self.is_connected(protocol).await {
            return Err(RequestToSv2ClientServiceError::IsNotConnected);
        }

        let tcp_client = match protocol {
            Protocol::MiningProtocol => match self.mining_tcp_client.lock().await.as_ref() {
                Some(client) => client.clone(),
                None => return Err(RequestToSv2ClientServiceError::IsNotConnected),
            },
            Protocol::JobDeclarationProtocol => {
            ...
            tokio::select! {
                ...
                message_result = tcp_client.io.recv_message() => {
                    match message_result {
                        Ok((message, _)) => {
                            if let Err(e) = self.call(RequestToSv2ClientService::Message(message)).await {
...
impl<T> Service<RequestToSv2ClientService<'static>> for Sv2ClientService<T>
where
    T: Sv2TemplateDistributionClientHandler + Clone + Send + Sync + 'static,
{
    type Response = ResponseFromSv2ClientService<'static>;
    type Error = RequestToSv2ClientServiceError;
    type Future =
    ...
    fn call(&mut self, request: RequestToSv2ClientService<'static>) -> Self::Future {
        let mut this = self.clone();

        Box::pin(async move {
            let response = match request {
                RequestToSv2ClientService::SetupConnectionTrigger(protocol) => match protocol {
                    Protocol::MiningProtocol => {
                        debug!(
                        ...
                        if this.config.mining_config.is_none() {
                            return Err(RequestToSv2ClientServiceError::UnsupportedProtocol {
                                protocol: Protocol::MiningProtocol,
                            });
                        }
                ...
                RequestToSv2ClientService::Message(sv2_message) => {
                    match sv2_message {
                        AnyMessage::Common(common) => match common {
                            CommonMessages::SetupConnection(_) => {
                                ...
                                Err(RequestToSv2ClientServiceError::UnsupportedMessage)
                            }
                            CommonMessages::SetupConnectionSuccess(_) => {
                                ...
                                Err(RequestToSv2ClientServiceError::BadRouting)
                            }
                ...
                RequestToSv2ClientService::TemplateDistributionTrigger(request) => match request {
                    RequestToSv2TemplateDistributionClientService::SetCoinbaseOutputConstraints(
                        ...
                        match result {
                            Ok(_) => {
                                ...
                                Ok(ResponseFromSv2ClientService::ResponseToTemplateDistributionTrigger(ResponseToTemplateDistributionTrigger::SuccessfullySetCoinbaseOutputConstraints))
                            }
                            Err(e) => Err(e.into()),
                        }
                    }
            ...
            let response =
                if let Ok(ResponseFromSv2ClientService::TriggerNewRequest(request)) = response {
...
mod tests {
    ...
    use crate::client::service::subprotocols::template_distribution::handler::NullSv2TemplateDistributionClientHandler;
    use crate::client::service::subprotocols::template_distribution::handler::Sv2TemplateDistributionClientHandler;
    use crate::client::service::RequestToSv2ClientServiceError;
    ...
    async fn sv2_client_service_initiate_connection_success() {
        ...
        let initiate_connection_request = RequestToSv2ClientService::SetupConnectionTrigger(
            Protocol::TemplateDistributionProtocol,
        );
